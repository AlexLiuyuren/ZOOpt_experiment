...
import numpy as np
from objective_function.high_dim_function import rastrigin_high_log, get_all_epoch, set_epoch_len, clear_noisy_global
...
def minimize_rastrigin_continuous():
    ...
    toolbox.register("evaluate", lambda x: (rastrigin_high_log(x), ))
    fmin=[]
    population = 20
...
if __name__ == '__main__':
    ...
    for i in range(repeat):
        minimize_rastrigin_continuous()
        print(i)
    all_epoch = np.array(get_all_epoch())
    np.savetxt('DEAP_exp/log/rastrigin/rastrigin_high_dim.txt', all_epoch)
