from objective_function.ordinary_function import griewank_log, clear_noisy_global, get_all_epoch, get_epoch_cnt, set_epoch_len
...
if __name__ == '__main__':
    ...
    for i in range(repeat_num):
        ...
        objective = Objective(lambda sol: griewank_log(sol.get_x()), dim)  # form up the objective function
        parameter = Parameter(budget=budget)
        sol = Opt.min(objective, parameter)
        sol.print_solution()
        clear_noisy_global()
    all_epoch = np.array(get_all_epoch())
    np.savetxt('ZOOpt_exp/log/griewank/griewank_20.txt', all_epoch)
