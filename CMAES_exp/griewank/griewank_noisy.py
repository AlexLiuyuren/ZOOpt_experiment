from objective_function.noisy_function import griewank_noise_for_cmaes, get_all_epoch, get_epoch_cnt, clear_noisy_global, set_epoch_len
...
def minimize_griewank_continuous_noisy():
    ...
    print(init_pos)
    es = cma.CMAEvolutionStrategy(init_pos, 3)  #doctest: +ELLIPSsIS
    nh = cma.NoiseHandler(es.N, maxevals=[1, 1, 30])
    while get_epoch_cnt() < 1:
         X, fit_vals = es.ask_and_eval(griewank_noise_for_cmaes, evaluations=nh.evaluations)
         es.tell(X, fit_vals)  # prepare for next iteration
         es.sigma *= nh(X, fit_vals, griewank_noise_for_cmaes, es.ask)  # see method __call__
...
if __name__ == '__main__':
    ...
    for i in range(repeat):
        sol = minimize_griewank_continuous_noisy()
    all_epoch = np.array(get_all_epoch())
    np.savetxt('CMAES_exp/log/griewank/griewank_noisy.txt', all_epoch)
