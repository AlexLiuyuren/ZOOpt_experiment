from objective_function.noisy_function import griewank_noise_log, get_all_epoch, get_epoch_cnt, clear_noisy_global, set_epoch_len
...
def minimize_griewank_continuous_noisy():
    ...
    while get_epoch_cnt() < 1:
         X, fit_vals = es.ask_and_eval(griewank_noise_log, evaluations=nh.evaluations)
         es.tell(X, fit_vals)  # prepare for next iteration
         es.sigma *= nh(X, fit_vals, griewank_noise_log, es.ask)  # see method __call__
