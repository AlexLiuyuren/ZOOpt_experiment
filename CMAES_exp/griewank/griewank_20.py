from objective_function.ordinary_function import griewank_for_cmaes, get_all_epoch, get_epoch_cnt, clear_noisy_global, set_epoch_len
...
def minimize_griewank():
    ...
    while get_epoch_cnt() < 1:
        solutions = es.ask()
        es.tell(solutions, [griewank_for_cmaes(x) for x in solutions])
        es.logger.add()
    clear_noisy_global()
