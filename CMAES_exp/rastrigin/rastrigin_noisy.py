...
def minimize_rastrigin_continuous_noisy():
    ...
    while get_epoch_cnt() < 1:
         X, fit_vals = es.ask_and_eval(rastrigin_noise_for_cmaes, evaluations=nh.evaluations)
         es.tell(X, fit_vals)  # prepare for next iteration
         es.sigma *= nh(X, fit_vals, rastrigin_noise_for_cmaes, es.ask)  # see method __call__
...
if __name__ == '__main__':
    repeat = 10
    set_optimal_position("objective_function/optimal_position/rastrigin/rastrigin_20.txt")
    set_epoch_len(40000)
