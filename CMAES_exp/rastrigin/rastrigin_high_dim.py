from objective_function.high_dim_function import rastrigin_high_for_cmaes, get_all_epoch, get_epoch_cnt, clear_noisy_global, set_epoch_len
...
def minimize_rastrigin():
    ...
    while get_epoch_cnt() < 1:
        solutions = es.ask()
        es.tell(solutions, [rastrigin_high_for_cmaes(x) for x in solutions])
        es.logger.add()
    clear_noisy_global()
